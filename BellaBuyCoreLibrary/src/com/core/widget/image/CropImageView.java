 ...
 *
 * @author sufun_job
 * @descritipon 用于普通的截取图片ImageView
 * 1）进行图片的旋转    mCropView.rotateImage(CropImageView.RotateDegrees.ROTATE_90D);
 * 2）自应用图片模式当截区域与图片大小一致  mCropView.setCropMode(CropImageView.CropMode.RATIO_FIT_IMAGE);
 * 3）图像的比例   RATIO_1_1   宽：高
 * 4）圆形头像的截取     CropImageView.CropMode.CIRCLE
 * 5）任意比例的头像截取     CropImageView.CropMode.RATIO_FREE
 * 6）取得截取后的结果数据    mCropView.getCroppedBitmap()   
 *
 *
 *  layout 参考： xmlns:custom="http://schemas.android.com/apk/res-auto
 *
            <com.core.widget.image.CropImageView
    ...
    色值参考：
<color name="base">#FFFFFB</color>
<color name="overlay">#AA1C1C1C</color>
<color name="text">#1C1C1C</color>
<color name="handle">#86C166</color>
<color name="frame">#86C166</color>
<color name="guide">#AA86C166</color>
<color name="menu_bg">#FFFFFB</color>
 */
public class CropImageView extends SFImageView {
